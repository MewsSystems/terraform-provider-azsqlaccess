...
//
// DefaultSchema is intentionally not read back. The search_path role setting
// lives in pg_db_role_setting and is awkward to retrieve reliably. Since
// DefaultSchema is Required (not Optional+Computed), the user-supplied value
// is canonical and state never needs to be overwritten from the DB.
