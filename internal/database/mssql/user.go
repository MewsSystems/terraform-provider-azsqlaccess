...
//
// ObjectID is intentionally NOT read back from the database. MSSQL stores GUIDs
// as mixed-endian (bytes 0-3, 4-5, 6-7 stored little-endian in the SID), so
// CONVERT(uniqueidentifier, sid) returns a byte-swapped UUID that does not match
// what the user provided. The user-supplied value is always canonical.
