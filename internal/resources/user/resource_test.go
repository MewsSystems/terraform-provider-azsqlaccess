...
// TestUserResource_ValidateConfig_GroupUnknownObjectID_Skipped guards the case
// where object_id is computed from another resource or data source (e.g.
...
// "absent" — only a null object_id should error.
func TestUserResource_ValidateConfig_GroupUnknownObjectID_Skipped(t *testing.T) {
	ctx := context.Background()
	schema := userSchema(t)
	...
	values["type"] = tftypes.NewValue(tftypes.String, "group")
	values["name"] = tftypes.NewValue(tftypes.String, "db.reader")
	...
	r.ValidateConfig(ctx, resource.ValidateConfigRequest{Config: cfg}, resp)
	if resp.Diagnostics.HasError() {
		t.Fatalf("ValidateConfig with type=group and unknown object_id must not error; got %v", resp.Diagnostics)
	}
}
