...
# type = group  (object_id is part of the ID — required field, can't be read from DB)
terraform import azsqlaccess_user.example \
...
SELECT
    ...
    type_desc,
    principal_id,
    CONVERT(uniqueidentifier, sid) AS object_id_bytes  -- note: byte-swapped, not canonical
