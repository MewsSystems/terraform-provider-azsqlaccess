...
- **Go:** see `go.mod` (currently 1.25.11).
...
- **Import ID format** is documented in the import-handler comment block on each resource and must match `Create()`'s ID assembly.
  ...
  - `object_id` is in the import ID for group/SP because the provider takes it from config/import and never reads it back from the database. (MSSQL can recover it from the SID via `CONVERT(uniqueidentifier, sid)`, and PostgreSQL stores it in a `pgaadauth` security label, but the provider relies on neither.)
...
- **Don't rely on reading `object_id` from the database.** The provider takes it from config or the import ID by design — nowhere else. (It's technically recoverable — MSSQL via `CONVERT(uniqueidentifier, sid)`, PostgreSQL from the `pgaadauth` security label — but the code doesn't, and shouldn't start.)
